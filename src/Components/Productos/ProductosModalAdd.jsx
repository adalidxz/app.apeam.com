...
import { getProducts, guardarProducto } from '../../Redux/productosDucks';
import { getListProveedores } from '../../Redux/proveedoresDucks';
import Modal from '../Modals/ModalForm';

export default function ProductosModalAdd({close}) {
    ...
    const saveProducto = async(e)=>{
        e.preventDefault();
        const state = await dispatch(guardarProducto(form))
        if(state===true){
            close();
